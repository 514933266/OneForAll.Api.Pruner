...
using Quartz.Impl;
using Quartz.Spi;
using Pruner.Host.Filters;
using Pruner.Host.Models;
using Pruner.Host.Providers;
...
namespace Pruner.Host
{
    public class Startup
    {
        ...
        private const string HTTP_SERVICE = "Pruner.HttpService";
        private const string HTTP_SERVICE_KEY = "HttpService";

        private const string BASE_HOST = "Pruner.Host";
        private const string BASE_DOMAIN = "Pruner.Domain";
        private const string BASE_APPLICATION = "Pruner.Application";
        private const string BASE_REPOSITORY = "Pruner.Repository";
        ...
        public void ConfigureContainer(ContainerBuilder builder)
        {
            ...
            builder.Register(p =>
            {
                var optionBuilder = new DbContextOptionsBuilder<DDSDbContext>();
                optionBuilder.UseSqlServer(Configuration["ConnectionStrings:Default"]);
                return optionBuilder.Options;
            }).AsSelf();

            builder.RegisterType<DDSDbContext>().Named<DbContext>("DDSDbContext");
            builder.RegisterAssemblyTypes(Assembly.Load(BASE_REPOSITORY))
               .Where(t => t.Name.EndsWith("Repository"))
               .WithParameter(ResolvedParameter.ForNamed<DbContext>("DDSDbContext"))
               .AsImplementedInterfaces();
        }
