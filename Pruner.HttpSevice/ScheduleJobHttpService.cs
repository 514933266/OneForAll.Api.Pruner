...
namespace Pruner.HttpService
{
    ...
    public class ScheduleJobHttpService : BaseHttpService, IScheduleJobHttpService
    {
        ...
        /// <returns结果</returns>
        public async Task<BaseErrType> LogAsync(string appId, string taskName, string log, bool isException)
        {
            var client = GetHttpClient(_config.SysJob);
            if (client != null && client.BaseAddress != null)
            {
                var response = await client.PostAsync($"api/ScheduleJobs/{appId}/{taskName}/Logs?isException={isException}", log, new JsonMediaTypeFormatter());
                var msg = await response.Content.ReadAsAsync<BaseMessage>();
                return msg.ErrType;
            }
            return BaseErrType.Fail;
